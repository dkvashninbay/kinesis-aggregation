...
from . import DIGEST_SIZE, MAGIC, MAX_BYTES_PER_RECORD, kpl_pb2
...
def _calculate_varint_size(value):
    ...
    Returns:
        The number of bytes necessary to represent
        the input value as a varint. (int)
    """
...
class KeySet(object):
    ...
    def get_potential_index(self, key):
        ...
        Returns:
            The integer index that this key
            would occupy if added to the KeySet. (int)
        """
    ...
    def contains(self, key):
        ...
        Args:
            key (str) - The key whose existence
            in the KeySet should be checked.
        Returns:
...
class RecordAggregator(object):
    ...
    def on_record_complete(self, callback, execute_on_new_thread=True):
        ...
        Args:
            ...
            on a separate thread every time a new aggregated record
            is available
            (function or callable object).

            execute_on_new_thread - True if callbacks should be
            executed on a new thread, False if it should be executed
            on the calling thread. Defaults to True. (boolean)
        """
    ...
    def get_num_user_records(self):
        """Returns:
            The number of user records currently aggregated in
            this aggregated record. (int)
        """
    ...
    def get_size_bytes(self):
        """Returns:
            The total number of bytes in this aggregated record
            (based on the size of the serialized record. (int)
        """

        return self.current_record.get_size_bytes()

    def clear_record(self):
        """Clear all the user records from this aggregated
        record and reset it to an empty state.
        """

        self.current_record = AggRecord()

    def clear_callbacks(self):
        """Clear all the callbacks from this object that were
        registered with the on_record_complete method.
        """
    ...
    def clear_and_get(self):
        ...
        (whether full or not) as a single record and then clear the contents
        of this object so it can be re-used.  This method is useful for
        flushing the aggregated record when you need to transmit it before
        it is full (e.g. you're shutting down or haven't
        transmitted in a while).

        Returns:
            A partially-filled AggRecord or None if the aggregator is empty.
            (AggRecord)
        """
    ...
    def add_user_record(self, partition_key, data, explicit_hash_key=None):
        """Add a new user record to this aggregated record (will trigger
        a callback via onRecordComplete if aggregated record is full).
        ...
        success = self.current_record.add_user_record(partition_key, data,
                                                      explicit_hash_key)
        if success:
        ...
        # Current record is full so clear it out, make a new empty one and add
        # the user record
        self.clear_record()
        self.current_record.add_user_record(partition_key, data,
                                            explicit_hash_key)
...
class AggRecord(object):
    ...
    def _calculate_record_size(self, partition_key, data,
        ...
        if explicit_hash_key is not None and not self.explicit_hash_keys.contains(
                explicit_hash_key):
        ...
        inner_record_size += _calculate_varint_size(
            self.partition_keys.get_potential_index(partition_key))
    ...
    def add_user_record(self, partition_key, data, explicit_hash_key=None):
        ...
        if explicit_hash_key is not None and isinstance(explicit_hash_key,
                                                        str):
            explicit_hash_key_bytes = explicit_hash_key.encode('utf-8')
        elif explicit_hash_key is None:
            explicit_hash_key_bytes = AggRecord._create_explicit_hash_key(
                partition_key_bytes)
        ...
        # PutRecordRequest
        size_of_new_record = self._calculate_record_size(
            partition_key_bytes, data_bytes, explicit_hash_key_bytes)
