...
from . import DIGEST_SIZE, MAGIC, kpl_pb2
...
def _create_user_record(ehks, pks, mr, r, sub_seq_num):
    ...
    new_record = {
        'kinesis': {
            ...
            # We actually re-base64-encode the data because that's how Lambda
            # Python normally delivers records
            'data': base64.b64encode(mr.data)
        }
    }

    # Copy all the metadata from the original record (except the data-specific
    # stuff)
...
def _get_error_string(r, message_data, ehks, pks, ar):
    ...
    error_buffer.write(
        'Unexpected exception during deaggregation, record was:\n')
