...
try:
    ...
    print(
        "The 'boto3' module is required to run this script. Use 'pip install boto3' to get it.",
        file=sys.stderr
    )
    sys.exit(1)
...
def send_record(agg_record):
    ...
    print(
        'Submitting record with EHK=%s NumRecords=%d NumBytes=%d' %
        (explicit_hash_key,
         agg_record.get_num_user_records(),
         agg_record.get_size_bytes())
    )
    try:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print(
            "USAGE: python kinesis_publisher.py <stream name> <region>",
            file=sys.stderr,
        )
        sys.exit(1)
